...
class UnsupervisedLoss1Logger(Callback):
	...
	batch_loss = np.mean(self._loss(X_batch, X_batch))
...
class UnsupervisedLoss2Logger(Callback):
    ...
    def compile(self, theano_mode=None):
        ...
        #self._dummy_model = Sequential()
        #self._predict_loop = self._dummy_model.predict
	...
	#print("aaaaaaaaaaaaaaaaaa")
        ret_val = self._predict_loop(self._loss, X, self.batch_size, verbose)
	#print("vvvvvvvvvvvvvvvvaaaa")
        return ret_val[0]

    def on_epoch_end(self, epoch, logs={}):
	#print("dddddddddddddddddddddddddddddd")
        if (epoch+1) % self.every_n_epochs == 0:
	    loss = np.mean(self._loss(self.X_train, self.X_test))
